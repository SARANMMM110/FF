...
app.get("/api/profile", authMiddleware, async (c) => {
    ...
    const { results } = await c.env.DB.prepare("SELECT * FROM user_profiles WHERE user_id = ?")
        .bind(user.id)
        .all();
    if (results.length === 0) {
        // Create default profile
        await c.env.DB.prepare(`INSERT INTO user_profiles (user_id, display_name) VALUES (?, ?)`)
            .bind(user.id, user.google_user_data?.name || "")
            .run();
        const { results: newResults } = await c.env.DB.prepare("SELECT * FROM user_profiles WHERE user_id = ?")
            .bind(user.id)
            .all();
...
app.patch("/api/profile", authMiddleware, zValidator("json", UpdateProfileSchema), async (c) => {
    ...
    const { results: existingResults } = await c.env.DB.prepare("SELECT * FROM user_profiles WHERE user_id = ?")
        .bind(user.id)
        .all();
    const updates = [];
    const values = [];
    ...
    if (existingResults.length === 0) {
        ...
        createFields.push("display_name", "bio", "phone", "address_line1", "address_line2", "city", "state", "country", "postal_code", "profile_photo_url", "website_url", "timezone", "date_of_birth", "occupation", "company");
        createValues.push(data.display_name || "", data.bio || "", data.phone || "", data.address_line1 || "", data.address_line2 || "", data.city || "", data.state || "", data.country || "", data.postal_code || "", data.profile_photo_url || "", data.website_url || "", data.timezone || "", data.date_of_birth || "", data.occupation || "", data.company || "");
        const placeholders = createFields.map(() => "?").join(", ");
        await c.env.DB.prepare(`INSERT INTO user_profiles (${createFields.join(", ")}) VALUES (${placeholders})`)
            .bind(...createValues)
            .run();
    }
    else {
        ...
        values.push(user.id);
        await c.env.DB.prepare(`UPDATE user_profiles SET ${updates.join(", ")} WHERE user_id = ?`)
            .bind(...values)
            .run();
    }
...
app.post("/api/profile/photo", authMiddleware, async (c) => {
    ...
    try {
        ...
        await c.env.DB.prepare("UPDATE user_profiles SET profile_photo_url = ?, updated_at = ? WHERE user_id = ?")
...
const adminMiddleware = async (c, next) => {
    ...
    const { results } = await c.env.DB.prepare(`SELECT au.*, ads.expires_at 
     FROM admin_users au 
     JOIN admin_sessions ads ON au.id = ads.admin_id 
     WHERE ads.session_token = ? AND ads.expires_at > datetime('now')`).bind(token).all();
    if (results.length === 0) {
        return c.json({ error: "Invalid or expired admin session" }, 401);
    }
    c.set("admin", results[0]);
    await next();
};
// Auth endpoints
app.get("/api/oauth/google/redirect_url", async (c) => {
    ...
    try {
        ...
        let redirectUrl = getGoogleOAuthRedirectUrl(c.env.GOOGLE_CALENDAR_CLIENT_ID, redirectUri, plan || undefined // Pass plan as state parameter if present
        );
...
app.post("/api/sessions", async (c) => {
    ...
    `).bind(registrationCode).all();
        ...
        if (results.length > 0) {
            ...
            if (isValid) {
                ...
                await c.env.DB.prepare("UPDATE registration_codes SET current_uses = current_uses + 1, updated_at = ? WHERE code = ?").bind(new Date().toISOString(), registrationCode).run();
    ...
    let googleUserInfo;
    try {
        const tokens = await exchangeGoogleCodeForTokens(body.code, c.env.GOOGLE_CALENDAR_CLIENT_ID, c.env.GOOGLE_CALENDAR_CLIENT_SECRET, redirectUri);
    ...
    if (user) {
        ...
        try {
            ...
            const userResult = await c.env.DB.prepare("SELECT user_id FROM users WHERE user_id = ? LIMIT 1").bind(user.id).all();
            userCheck = userResult.results || [];
            const settingsResult = await c.env.DB.prepare("SELECT user_id FROM user_settings WHERE user_id = ? LIMIT 1").bind(user.id).all();
        ...
        let signupCheck = [];
        try {
            const signupResult = await c.env.DB.prepare("SELECT email FROM email_signups WHERE email = ? LIMIT 1").bind(user.email).all();
        ...
        if (isNewUser) {
            ...
            try {
                await c.env.DB.prepare(`INSERT INTO users (user_id, email, name, google_user_id, profile_picture_url, signup_source, subscription_plan, last_login_at)
           VALUES (?, ?, ?, ?, ?, 'google-oauth', ?, ?)`).bind(user.id, user.email, user.google_user_data?.name || null, user.google_user_data?.sub || null, user.google_user_data?.picture || null, subscriptionPlan, new Date().toISOString()).run();
            }
            catch (dbError) {
            ...
            const { results: verifyResults } = await c.env.DB.prepare("SELECT user_id, email, subscription_plan FROM users WHERE user_id = ?").bind(user.id).all();
            if (verifyResults.length > 0) {
                const savedUser = verifyResults[0];
            ...
            await c.env.DB.prepare("UPDATE users SET last_login_at = ?, updated_at = ? WHERE user_id = ?").bind(new Date().toISOString(), new Date().toISOString(), user.id).run();
            ...
            if (specialPlan && registrationCode) {
                ...
                // Get current plan
                const { results: currentUserData } = await c.env.DB.prepare("SELECT subscription_plan FROM users WHERE user_id = ?").bind(user.id).all();
                const currentPlan = currentUserData.length > 0 ? currentUserData[0].subscription_plan : 'free';
                ...
                if (newLevel > currentLevel) {
                    ...
                    await c.env.DB.prepare("UPDATE users SET subscription_plan = ?, updated_at = ? WHERE user_id = ?").bind(specialPlan, new Date().toISOString(), user.id).run();
                    ...
                    // Verify the upgrade
                    const { results: verifyUpgrade } = await c.env.DB.prepare("SELECT subscription_plan FROM users WHERE user_id = ?").bind(user.id).all();
                    if (verifyUpgrade.length > 0) {
                        const updatedPlan = verifyUpgrade[0].subscription_plan;
        ...
        if (isNewUser || notInWaitlist) {
            ...
            if (notInWaitlist) {
                try {
                    await c.env.DB.prepare(`INSERT INTO email_signups (email, name, signup_source, marketing_consent)
             VALUES (?, ?, 'google-oauth', 1)`).bind(user.email, user.google_user_data?.name || "").run();
        ...
        if (settingsCheck.length === 0) {
            try {
                await c.env.DB.prepare("INSERT INTO user_settings (user_id) VALUES (?)").bind(user.id).run();
...
app.post("/api/admin/login", zValidator("json", AdminLoginSchema), async (c) => {
    ...
    const { results } = await c.env.DB.prepare("SELECT * FROM admin_users WHERE username = ?").bind(username).all();
    if (results.length === 0) {
    ...
    await c.env.DB.prepare("INSERT INTO admin_sessions (admin_id, session_token, expires_at) VALUES (?, ?, ?)").bind(admin.id, sessionToken, expiresAt).run();
...
app.get("/api/admin/stats", adminMiddleware, async (c) => {
    ...
    // Total tasks
    const { results: taskCount } = await c.env.DB.prepare("SELECT COUNT(*) as count FROM tasks").all();
    // Total focus sessions
    const { results: sessionCount } = await c.env.DB.prepare("SELECT COUNT(*) as count FROM focus_sessions").all();
    // Total focus time
    const { results: focusTime } = await c.env.DB.prepare("SELECT COALESCE(SUM(duration_minutes), 0) as total FROM focus_sessions WHERE session_type = 'focus' AND end_time IS NOT NULL").all();
    // Active users (users with activity in last 7 days)
    const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
    const { results: activeUsers } = await c.env.DB.prepare("SELECT COUNT(DISTINCT user_id) as count FROM users WHERE last_login_at >= ?").bind(sevenDaysAgo).all();
    return c.json({
        total_users: userCount[0].count,
        total_tasks: taskCount[0].count,
        total_sessions: sessionCount[0].count,
        total_focus_minutes: focusTime[0].total,
...
app.get("/api/admin/users", adminMiddleware, async (c) => {
  ...
  `).bind(limit, offset).all();
    ...
    const { results: totalCount } = await c.env.DB.prepare("SELECT COUNT(*) as count FROM users").all();
    return c.json({
        users,
        pagination: {
            page,
...
app.get("/api/admin/users/:userId", adminMiddleware, async (c) => {
    ...
    // Get user tasks
    const { results: tasks } = await c.env.DB.prepare("SELECT * FROM tasks WHERE user_id = ? ORDER BY created_at DESC").bind(userId).all();
    // Get user focus sessions
    const { results: sessions } = await c.env.DB.prepare("SELECT * FROM focus_sessions WHERE user_id = ? ORDER BY start_time DESC LIMIT 50").bind(userId).all();
    // Get user settings
    const { results: settings } = await c.env.DB.prepare("SELECT * FROM user_settings WHERE user_id = ?").bind(userId).all();
    return c.json({
        user_id: userId,
        tasks,
        sessions,
...
app.post("/api/admin/users/:userId/tasks", adminMiddleware, zValidator("json", AdminCreateTaskSchema), async (c) => {
    const userId = c.req.param("userId");
    const data = c.req.valid("json");
    const result = await c.env.DB.prepare(`INSERT INTO tasks (user_id, title, description, priority, estimated_minutes, project, due_date, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'todo')`)
        .bind(userId, data.title, data.description || null, data.priority || 0, data.estimated_minutes || null, data.project || null, data.due_date || null)
        .run();
...
app.post("/api/admin/registration-codes", adminMiddleware, async (c) => {
    ...
    const admin = c.get("admin");
    try {
        await c.env.DB.prepare(`INSERT INTO registration_codes (code, plan_id, max_uses, expires_at, created_by, notes)
       VALUES (?, ?, ?, ?, ?, ?)`).bind(code, planId, maxUses, expiresAt, admin.username, notes).run();
        return c.json({
            success: true,
            code,
            plan_id: planId,
...
app.get("/api/admin/registration-codes", adminMiddleware, async (c) => {
    const { results } = await c.env.DB.prepare("SELECT * FROM registration_codes ORDER BY created_at DESC LIMIT 100").all();
...
app.delete("/api/admin/registration-codes/:code", adminMiddleware, async (c) => {
    ...
    await c.env.DB.prepare("UPDATE registration_codes SET is_active = 0, updated_at = ? WHERE code = ?").bind(new Date().toISOString(), code).run();
    return c.json({ success: true });
});
...
app.patch("/api/admin/users/:userId/plan", adminMiddleware, async (c) => {
    ...
    try {
        ...
        const { results: userCheck } = await c.env.DB.prepare("SELECT user_id FROM users WHERE user_id = ?").bind(userId).all();
        ...
        await c.env.DB.prepare("UPDATE users SET subscription_plan = ?, updated_at = ? WHERE user_id = ?").bind(planId, new Date().toISOString(), userId).run();
        return c.json({
            success: true,
...
app.get("/api/tasks", authMiddleware, async (c) => {
    const user = c.get("user");
    const { results } = await c.env.DB.prepare("SELECT * FROM tasks WHERE user_id = ? ORDER BY priority DESC, created_at DESC")
        .bind(user.id)
        .all();
...
app.post("/api/tasks", authMiddleware, zValidator("json", CreateTaskSchema), async (c) => {
    ...
    const result = await c.env.DB.prepare(`INSERT INTO tasks (user_id, title, description, priority, estimated_minutes, project, due_date, tags, status, repeat, repeat_detail, next_occurrence_date)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'todo', ?, ?, ?)`)
        .bind(user.id, data.title, data.description || null, data.priority || 0, data.estimated_minutes || null, data.project || null, data.due_date || null, tagsJson, repeat, repeatDetail, nextOccurrence)
        .run();
    const { results } = await c.env.DB.prepare("SELECT * FROM tasks WHERE id = ?")
        .bind(result.meta.last_row_id)
        .all();
...
app.patch("/api/tasks/:id", authMiddleware, zValidator("json", UpdateTaskSchema), async (c) => {
    ...
    const id = c.req.param("id");
    const data = c.req.valid("json");
    const { results: existing } = await c.env.DB.prepare("SELECT * FROM tasks WHERE id = ? AND user_id = ?")
        .bind(id, user.id)
        .all();
    ...
    if (data.is_completed && updatedTask) {
        await syncToNotionIfEnabled(c.env.DB, user.id, updatedTask);
    }
...
app.get("/api/tasks/:taskId/subtasks", authMiddleware, async (c) => {
    ...
    const { results: taskResults } = await c.env.DB.prepare("SELECT * FROM tasks WHERE id = ? AND user_id = ?")
        .bind(taskId, user.id)
        .all();
    if (taskResults.length === 0) {
        return c.json({ error: "Task not found" }, 404);
    }
    const { results } = await c.env.DB.prepare("SELECT * FROM subtasks WHERE task_id = ? ORDER BY position ASC, created_at ASC")
        .bind(taskId)
        .all();
    return c.json(results);
});
app.post("/api/tasks/:taskId/subtasks", authMiddleware, zValidator("json", CreateSubtaskSchema), async (c) => {
    ...
    const result = await c.env.DB.prepare(`INSERT INTO subtasks (task_id, title, estimated_minutes, position)
     VALUES (?, ?, ?, ?)`)
...
app.patch("/api/subtasks/:id", authMiddleware, zValidator("json", UpdateSubtaskSchema), async (c) => {
    ...
    values.push(new Date().toISOString());
    values.push(id);
    await c.env.DB.prepare(`UPDATE subtasks SET ${updates.join(", ")} WHERE id = ?`)
        .bind(...values)
        .run();
    const { results } = await c.env.DB.prepare("SELECT * FROM subtasks WHERE id = ?")
        .bind(id)
...
app.post("/api/focus-sessions", authMiddleware, zValidator("json", CreateFocusSessionSchema), async (c) => {
    ...
    const result = await c.env.DB.prepare(`INSERT INTO focus_sessions (user_id, task_id, start_time, session_type, timer_mode)
     VALUES (?, ?, ?, ?, ?)`)
...
app.patch("/api/focus-sessions/:id", authMiddleware, zValidator("json", UpdateFocusSessionSchema), async (c) => {
    ...
    const { results: existing } = await c.env.DB.prepare("SELECT * FROM focus_sessions WHERE id = ?")
        .bind(id)
        .all();
    if (existing.length === 0) {
    ...
    values.push(new Date().toISOString());
    values.push(id, user.id);
    await c.env.DB.prepare(`UPDATE focus_sessions SET ${updates.join(", ")} WHERE id = ? AND user_id = ?`)
        .bind(...values)
        .run();
...
app.get("/api/settings", authMiddleware, async (c) => {
    ...
    if (results.length === 0) {
        ...
        await c.env.DB.prepare(`INSERT INTO user_settings (user_id) VALUES (?)`)
            .bind(user.id)
            .run();
        const { results: newResults } = await c.env.DB.prepare("SELECT * FROM user_settings WHERE user_id = ?")
            .bind(user.id)
            .all();
        return c.json(newResults[0]);
    }
...
app.get("/api/user/subscription", authMiddleware, async (c) => {
    const user = c.get("user");
    try {
        const { results } = await c.env.DB.prepare("SELECT subscription_plan FROM users WHERE user_id = ?").bind(user.id).all();
...
app.get("/api/dashboard-stats", authMiddleware, async (c) => {
  ...
  `).bind(user.id).all();
    ...
    longestStreak = Math.max(longestStreak, currentStreak);
    return c.json({
        today_focus_minutes: todayResults[0].total_minutes,
        week_focus_minutes: weekResults[0].total_minutes,
        completed_today: completedResults[0].count,
...
app.get("/api/goals", authMiddleware, async (c) => {
    const user = c.get("user");
    const { results } = await c.env.DB.prepare("SELECT * FROM user_goals WHERE user_id = ? ORDER BY created_at DESC").bind(user.id).all();
    // Calculate current values for each goal
    const goalsWithProgress = await Promise.all(results.map(async (goal) => {
        let currentValue = 0;
        ...
        `).bind(user.id, goal.start_date, goal.end_date).all();
                currentValue = minutesResults[0].total;
                break;
        ...
        `).bind(user.id, goal.start_date, goal.end_date).all();
                currentValue = tasksResults[0].total;
                break;
            case "focus_sessions":
          ...
          WHERE user_id = ? AND session_type = 'focus' AND end_time IS NOT NULL
            AND start_time >= ? AND start_time <= ?
        `).bind(user.id, goal.start_date, goal.end_date).all();
                currentValue = sessionsResults[0].total;
                break;
            case "daily_streak":
        ...
        await c.env.DB.prepare("UPDATE user_goals SET current_value = ?, updated_at = ? WHERE id = ?").bind(currentValue, new Date().toISOString(), goal.id).run();
        // Check if goal is completed
        if (currentValue >= goal.target_value && !goal.is_completed) {
            await c.env.DB.prepare("UPDATE user_goals SET is_completed = 1, completed_at = ?, updated_at = ? WHERE id = ?").bind(new Date().toISOString(), new Date().toISOString(), goal.id).run();
            goal.is_completed = 1;
...
app.post("/api/goals", authMiddleware, zValidator("json", CreateGoalSchema), async (c) => {
    const user = c.get("user");
    const data = c.req.valid("json");
    const result = await c.env.DB.prepare(`INSERT INTO user_goals (user_id, title, description, target_type, target_value, start_date, end_date)
     VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(user.id, data.title, data.description || null, data.target_type, data.target_value, data.start_date, data.end_date).run();
...
app.patch("/api/goals/:id", authMiddleware, zValidator("json", UpdateGoalSchema), async (c) => {
    ...
    const id = c.req.param("id");
    const data = c.req.valid("json");
    const { results: existing } = await c.env.DB.prepare("SELECT * FROM user_goals WHERE id = ? AND user_id = ?").bind(id, user.id).all();
    if (existing.length === 0) {
    ...
    values.push(new Date().toISOString());
    values.push(id, user.id);
    await c.env.DB.prepare(`UPDATE user_goals SET ${updates.join(", ")} WHERE id = ? AND user_id = ?`).bind(...values).run();
    const { results } = await c.env.DB.prepare("SELECT * FROM user_goals WHERE id = ?")
        .bind(id)
        .all();
    return c.json(results[0]);
...
app.post("/api/email-signup", zValidator("json", EmailSignupSchema), async (c) => {
    ...
    try {
        ...
        const { results: existing } = await c.env.DB.prepare("SELECT * FROM email_signups WHERE email = ?").bind(data.email).all();
        if (existing.length > 0) {
            return c.json({ error: "Email already registered" }, 400);
        }
        // Save to database
        await c.env.DB.prepare(`INSERT INTO email_signups 
       (email, name, signup_source, marketing_consent, ip_address, user_agent, referrer, utm_source, utm_medium, utm_campaign)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(data.email, data.name || null, data.signup_source, data.marketing_consent ? 1 : 0, clientIP, userAgent, data.referrer || null, data.utm_source || null, data.utm_medium || null, data.utm_campaign || null).run();
...
app.get("/api/admin/email-signups", adminMiddleware, async (c) => {
    ...
    LIMIT ? OFFSET ?
  `).bind(limit, offset).all();
    const { results: totalCount } = await c.env.DB.prepare("SELECT COUNT(*) as count FROM email_signups").all();
    return c.json({
        signups,
        pagination: {
            page,
...
app.post("/api/test/create-session", authMiddleware, async (c) => {
    ...
    try {
        const result = await c.env.DB.prepare(`INSERT INTO focus_sessions (user_id, start_time, end_time, duration_minutes, session_type, timer_mode)
       VALUES (?, ?, ?, ?, 'focus', 'pomodoro')`).bind(user.id, thirtyMinsAgo.toISOString(), now.toISOString(), 30).run();
...
app.post("/api/focus-distractions", authMiddleware, zValidator("json", RecordDistractionSchema), async (c) => {
    const user = c.get("user");
    const data = c.req.valid("json");
    await c.env.DB.prepare(`INSERT INTO focus_distractions (user_id, session_id, distraction_type, duration_seconds, metadata, timestamp)
     VALUES (?, ?, ?, ?, ?, ?)`).bind(user.id, data.session_id || null, data.distraction_type, data.duration_seconds, data.metadata || null, new Date().toISOString()).run();
...
app.get("/api/focus-distractions/analytics", authMiddleware, async (c) => {
  ...
  `).bind(user.id, startDate.toISOString()).all();
    ...
    const blockedSiteStats = blockedSiteResults.map((row) => {
        try {
  ...
  `).bind(user.id, startDate.toISOString()).all();
    return c.json({
        ...
        total_seconds: totalResults[0]?.total_seconds || 0,
    ...
    if (code && !error) {
        c.executionCtx.waitUntil((async () => {
            try {
                ...
                const { results: existing } = await c.env.DB.prepare("SELECT id FROM user_calendar_connections WHERE user_id = ? AND provider = 'google'").bind(user.id).all();
                if (existing.length > 0) {
            ...
            `).bind(tokens.access_token, tokens.refresh_token || null, expiresAt, new Date().toISOString(), user.id).run();
                }
                else {
            ...
            `).bind(user.id, tokens.access_token, tokens.refresh_token || null, expiresAt).run();
                }
...
app.delete("/api/calendar/disconnect", authMiddleware, async (c) => {
    const user = c.get("user");
    await c.env.DB.prepare("DELETE FROM user_calendar_connections WHERE user_id = ? AND provider = 'google'").bind(user.id).run();
    return c.json({ success: true });
});
async function refreshCalendarToken(env, userId) {
  ...
  `).bind(userId).all();
    ...
    try {
      ...
      WHERE user_id = ? AND provider = 'google'
    `).bind(tokens.access_token, newExpiresAt, new Date().toISOString(), userId).run();
...
app.post("/api/recurring-tasks/process", authMiddleware, async (c) => {
    ...
    `).bind(user.id, today).all();
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, 'todo', ?, ?)
      `).bind(task.user_id, task.title, task.description, task.priority, task.estimated_minutes, task.project, task.tags, newDueDate, task.id).run();
            created++;
            // Calculate and update next occurrence
            const nextOccurrence = calculateNextOccurrence(newDueDate, task.repeat, task.repeat_detail);
            await c.env.DB.prepare(`
        UPDATE tasks 
        SET next_occurrence_date = ?, updated_at = ?
        WHERE id = ?
      `).bind(nextOccurrence, new Date().toISOString(), task.id).run();
        }
...
app.get("/api/export/tasks", authMiddleware, async (c) => {
    const user = c.get("user");
    const { results } = await c.env.DB.prepare("SELECT * FROM tasks WHERE user_id = ? ORDER BY created_at DESC").bind(user.id).all();
    // Generate CSV
...
async function mergeContiguousSessions(db, userId, taskId) {
  ...
  `).bind(userId, taskId).all();
    ...
    for (const group of toMerge) {
      ...
      WHERE id = ?
    `).bind(lastSession.end_time, totalDuration, allNotes || null, new Date().toISOString(), firstSession.id).run();
...
async function updateTaskActualTime(db, taskId) {
...
async function syncToNotionIfEnabled(db, userId, task) {
    // Check if Notion sync is enabled for this user
    const { results } = await db.prepare("SELECT notion_sync_enabled, notion_database_id, notion_access_token FROM user_settings WHERE user_id = ?").bind(userId).all();
    if (results.length === 0)
        return;
...
async function integrateWithSystemeIO(apiKey, signupData) {
    ...
    if (signupData.utm_data?.source)
        tags.push(`utm_source:${signupData.utm_data.source}`);
    if (signupData.utm_data?.medium)
        tags.push(`utm_medium:${signupData.utm_data.medium}`);
    if (signupData.utm_data?.campaign)
        tags.push(`utm_campaign:${signupData.utm_data.campaign}`);
    const payload = {
